"""Rulebook v8/v9 risk gate calculations."""
...
# Rule 2: VIX → EL retain % / max deploy %
...
# v9 table (PROPOSED — awaiting Yash promotion):
...
# Active table — promoted to V9 per audit-recommended survival bunker (2026-04-07)
RULEBOOK_VERSION = 'v9'
_VIX_EL_TABLE = _VIX_EL_TABLE_V9


def vix_required_el_pct(vix: float) -> float:
    """Rule 2: VIX → required EL retain % of NLV."""
...
LEVERAGE_LIMIT = 1.50
LEVERAGE_RELEASE = 1.40  # hysteresis — must drop below this to release
...
def dynamic_exit_threshold(
    ...
    Three regimes:
      ...
      3. d* > 25%: strong exit signal — deep drawdown, high opportunity cost
    """
    if wait_months <= 0:
        return 0.0
    if redeploy_yield <= 0:
