...
async def handle_dashboard_callback(
    ...
    """Unified callback handler for all dashboard inline buttons."""
    ...
    if not cache:
        await query.answer("Session expired. Please run the command again.",
                           show_alert=True)
        return

    # TTL check
    age = (_datetime.now() - cache["created_at"]).total_seconds()
    if age > DASHBOARD_TTL:
        dashboard_cache.pop(chat_id, None)
        await query.answer("Session expired. Please run the command again.",
                           show_alert=True)
        return

    view_html = cache["views"].get(view_key)
    if not view_html:
    ...
    try:
        ...
        # "Message is not modified" — user tapped same button, harmless
        pass
...
async def _legacy_handle_cc_ladder_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE,
) -> None:
    query = update.callback_query
    user = update.effective_user
    ...
    if len(parts) < 3:
        await query.answer("Malformed ladder action.", show_alert=True)
        return
    ...
    if action in {"confirm", "cancel"}:
        token = parts[2]
        pending = cc_confirmation_cache.get(token)
        if not pending:
            await query.answer("Confirmation expired.", show_alert=True)
            return

        age_seconds = (_datetime.now() - pending["created_at"]).total_seconds()
        if age_seconds > DASHBOARD_TTL or pending.get("chat_id") != chat_id:
            cc_confirmation_cache.pop(token, None)
            await query.answer("Confirmation expired.", show_alert=True)
            return

        if action == "cancel":
            cc_confirmation_cache.pop(token, None)
            await query.answer("Staging cancelled.")
            try:
                await query.edit_message_text("Covered call staging cancelled.")
            except Exception:
                pass
            return
        ...
        ticket = {
            ...
            "status": "pending",
            "transmit": False,
            "strategy": "Covered Call Ladder",
        }
        await asyncio.to_thread(append_pending_tickets, [ticket])
        cc_confirmation_cache.pop(token, None)

        await query.answer("Covered call staged.")
        try:
            strike_label = _format_strike_label(float(pending["strike"]))
            await query.edit_message_text(
                "Covered call staged: "
                f"SELL {int(pending['quantity'])}x {pending['ticker']} "
                f"{strike_label}C Exp {pending['expiry']} @ "
                f"${float(pending['premium']):.2f} LMT. "
                "Queued in agt_desk.db with transmit=False."
            )
        except Exception:
            pass
        return
    ...
    cache = dashboard_cache.get(chat_id)
    if not cache or cache.get("tool") != "run_cc_ladder":
        await query.answer("Ladder session expired. Please run it again.", show_alert=True)
        return
    ...
    data = cache.get("_raw_data", {})
    expirations = data.get("expirations", [])
    state = cache.setdefault("cc_state", {"expiry_index": 0, "page": 0})
    expiry_index = int(state.get("expiry_index", 0))
    page = int(state.get("page", 0))

    if action == "page":
        if not expirations:
            await query.answer("No ladder data available.", show_alert=True)
            return

        current_rows = expirations[expiry_index].get("rows", [])
        total_pages = max(1, math.ceil(len(current_rows) / 5))
        next_page = page + 1
        if next_page >= total_pages:
            await query.answer("No more strikes in this expiry.", show_alert=True)
            return
        ...
        try:
            target_expiry = int(parts[2])
        except ValueError:
            await query.answer("Invalid expiry selection.", show_alert=True)
            return

        if target_expiry < 0 or target_expiry >= len(expirations):
            await query.answer("Expiry is no longer available.", show_alert=True)
            return

        state["expiry_index"] = target_expiry
        state["page"] = 0

    elif action == "select":
        if len(parts) != 4:
            await query.answer("Malformed strike selection.", show_alert=True)
            return

        try:
            selected_expiry = int(parts[2])
            row_index = int(parts[3])
        except ValueError:
            await query.answer("Invalid strike selection.", show_alert=True)
            return

        if selected_expiry < 0 or selected_expiry >= len(expirations):
            await query.answer("Expiry is no longer available.", show_alert=True)
            return

        rows = expirations[selected_expiry].get("rows", [])
        if row_index < 0 or row_index >= len(rows):
            await query.answer("Strike is no longer available.", show_alert=True)
            return

        row = rows[row_index]
        strike = float(row["strike"])
        premium = float(row["premium"])
        quantity = int(data.get("covered_call_contracts", 0))
        if quantity <= 0:
            await query.answer("No covered-call capacity is available.", show_alert=True)
            return

        token = secrets.token_hex(6)
        cc_confirmation_cache[token] = {
            "chat_id": chat_id,
            "account_id": data["account_id"],
            "account_label": data.get("account_label", data["account_id"]),
            "ticker": data["ticker"],
            "quantity": quantity,
            "expiry": expirations[selected_expiry]["date"],
            "strike": strike,
            "premium": premium,
            "created_at": _datetime.now(),
        }
        ...
        confirm_text = (
            f"Confirm staging: SELL {quantity}x {data['ticker']} "
            f"{strike_label}C Exp {expirations[selected_expiry]['date']} "
            f"@ ${premium:.2f} LMT."
        )

        await query.answer("Awaiting confirmation.")
        await query.message.reply_text(
            text=confirm_text,
            reply_markup=_cc_confirm_keyboard(token),
        )
        return

    else:
        await query.answer("Unknown ladder action.", show_alert=True)
        return
    ...
    try:
        html_text, keyboard = _build_cc_ladder_views(
            data,
            expiry_index=int(state["expiry_index"]),
            page=int(state["page"]),
        )
        await query.edit_message_text(
            text=html_text,
        ...
        cache["created_at"] = _datetime.now()
    except Exception as exc:
        logger.exception("CC ladder callback failed")
        await query.answer(f"Ladder update failed: {exc}", show_alert=True)
...
async def handle_cc_ladder_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE,
) -> None:
    query = update.callback_query
    user = update.effective_user
    ...
    if action in {"confirm", "cancel"}:
        ...
        tickets, split_lines, remaining = _build_household_call_tickets(
            pending,
            int(pending["quantity"]),
            "Covered Call Ladder",
        )

        if not tickets or remaining > 0:
            cc_confirmation_cache.pop(token, None)
            await query.answer("No unencumbered household capacity remains.", show_alert=True)
            try:
                await query.edit_message_text("Covered call staging failed: no household capacity remains.")
            except Exception:
                pass
            return
        ...
        await query.answer("Covered calls staged.")
        try:
            strike_label = _format_strike_label(float(pending["strike"]))
            split_summary = "\n".join(split_lines)
            await query.edit_message_text(
                "Covered calls staged:\n"
                f"SELL {int(pending['quantity'])}x {pending['ticker']} "
                f"{strike_label}C Exp {pending['expiry']} @ "
                f"${float(pending['premium']):.2f} LMT.\n\n"
                f"{split_summary}\n\n"
    ...
    data = cache.get("_raw_data", {})
    expirations = data.get("expirations", [])
    state = cache.setdefault("cc_state", {"expiry_index": 0, "strike_offset": 0})
    expiry_index = int(state.get("expiry_index", 0))
    strike_offset = int(state.get("strike_offset", 0))

    if action == "page":
        if len(parts) != 4:
            await query.answer("Malformed ladder pagination.", show_alert=True)
            return

        try:
            expiry_index = int(parts[2])
            strike_offset = max(int(parts[3]), 0)
        except ValueError:
            await query.answer("Invalid ladder pagination.", show_alert=True)
            return

        data = await _reload_cc_ladder_data(data, strike_offset)
        state["expiry_index"] = expiry_index
        state["strike_offset"] = strike_offset

    elif action == "exp":
        if len(parts) != 4:
            await query.answer("Malformed expiry selection.", show_alert=True)
            return
        ...
        try:
            selected_expiry = int(parts[2])
            selected_offset = max(int(parts[3]), 0)
            row_offset = int(parts[4])
        except ValueError:
            await query.answer("Invalid strike selection.", show_alert=True)
            return
        ...
        quantity = sum(
            int(account_data.get("supported_contracts", 0))
            for account_data in inventory.values()
        )
        if quantity <= 0:
            await query.answer("No covered-call capacity is available.", show_alert=True)
            return
        ...
        state["expiry_index"] = selected_expiry
        state["strike_offset"] = selected_offset
    ...
    try:
        html_text, keyboard = _build_cc_ladder_views(
            data,
            expiry_index=int(state["expiry_index"]),
            strike_offset=int(state["strike_offset"]),
        )
        await query.edit_message_text(
            text=html_text,
            parse_mode="HTML",
...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    status_msg = await update.message.reply_text(
        "\U0001f50d Scanning approved watchlist for CSP candidates (3-10 DTE, 30%+ yield)..."
    )

    try:
        from pxo_scanner import scan_csp_candidates

        candidates = await asyncio.to_thread(scan_csp_candidates, None, 5)

        # ── Query per-account margin stats ──
        mstats = await _query_margin_stats()
        margin_error = mstats["error"]

        if not candidates:
            await status_msg.edit_text(
                "\U0001f50d <b>PXO Scanner — No Candidates</b>\n\n"
                "<pre>No CSP candidates met the Heitkoetter criteria\n"
                "(30%+ annualized, 3-10 DTE, 3%+ OTM, $0.10+ premium)\n"
                "across the approved watchlist.</pre>",
                parse_mode="HTML",
            )
            return

        # ── Build the Project & Paste payload ──
        lines = [
            "## CIO Payload: CSP Entry Scan",
            "",
            "**Trade Type:** CSP_ENTRY",
            f"**Scan Date:** {_date.today().isoformat()}",
            f"**DTE Window:** 3-10 days",
            f"**Yield Floor:** 30% annualized (Heitkoetter)",
            "",
            "### Available Capital",
            *(
                [f"**IB WARNING:** {margin_error}", ""]
                if margin_error else []
            ),
        ]
        for acct_id, ad in mstats["accounts"].items():
            lines.extend([
                f"**{ad['name']}**",
                f"- Net Liq: ${ad['nlv']:,.2f}",
                f"- Available Margin (EL): ${ad['el']:,.2f}",
                f"- Current EL %: {ad['el_pct']:.1f}%",
            ])
        lines.extend([
            "",
            "### Top CSP Candidates",
            "",
            "| Ticker | Strike | DTE | Premium | Ann ROI | OTM% | Cap Req |",
            "|--------|--------|-----|---------|---------|------|---------|",
        ])
        ...
        for c in candidates:
            lines.append(
                ...
                f"| {c['ann_roi']:.1f}% "
                f"| {c['otm_pct']:.1f}% "
                f"| ${c['capital_required']:,.0f} |"
            )

        # ── Fundamental Context: latest headline per ticker ──
        ...
        for c in candidates:
            ...
            hl = c.get("headline", "(no headline)")
            headline_lines.append(f"- **{tkr}:** \"{hl}\"")
        ...
        lines.append("")
        lines.append(
            "**All candidates pass:** OTM >= 3%, "
            "Premium >= $0.10, 30% <= Ann ROI <= 130%. "
            "Exclusions enforced (no Biotech/Pharma/China/Meme/Airlines)."
        )
        ...
        logger.info(
            "PXO scan complete: %d candidates, agg_margin=$%.2f",
            len(candidates),
            mstats["agg_margin_el"],
        )

    except Exception as exc:
        logger.exception("PXO scan failed")
        try:
            await status_msg.edit_text(
                f"\U0001f50d <b>PXO Scanner — Error</b>\n\n"
...
async def cmd_sync_universe(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Refresh the ticker_universe table from Wikipedia + yfinance."""
    if not is_authorized(update):
        return
    try:
        status_msg = await update.message.reply_text(
            "Refreshing ticker universe from S&P 500 + NASDAQ-100...\n"
            "This takes ~30-60 seconds (yfinance enrichment)."
        )
        result = await _refresh_ticker_universe()

        if result["error"]:
            await status_msg.edit_text(
                f"Universe refresh completed with errors:\n"
                f"Added: {result['added']} | Updated: {result['updated']}\n"
                f"Error: {result['error']}"
            )
        else:
            await status_msg.edit_text(
                f"Ticker universe refreshed.\n"
                f"Added: {result['added']} | Updated: {result['updated']} | "
                f"Total: {result['total']}"
            )
    except Exception as exc:
        logger.exception("cmd_sync_universe failed")
        try:
            await update.message.reply_text(f"Universe sync failed: {exc}")
        except Exception:
            pass
...
async def cmd_cleanup_blotter(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Purge stale live_blotter records that no longer match IBKR open orders."""
    if not is_authorized(update):
        return
    try:
        status_msg = await update.message.reply_text("Cleaning stale blotter records...")
        result = await _cleanup_stale_blotter()

        if result["error"]:
            await status_msg.edit_text(
                f"Blotter cleanup error: {result['error']}"
            )
        else:
            await status_msg.edit_text(
                f"Blotter cleanup done.\n"
                f"Checked: {result['checked']} | "
                f"Removed: {result['removed']}"
            )
    except Exception as exc:
        logger.exception("cmd_cleanup_blotter failed")
        try:
    ...
    try:
        ...
        with open(perf_path, "rb") as f:
            await update.message.reply_photo(
                photo=f,
                caption=f"\U0001f4c8 Performance Dashboard  ({source})",
            )
        ...
        if pos_path:
            with open(pos_path, "rb") as f:
                await update.message.reply_photo(
                    photo=f,
                    caption=f"\U0001f4cb Active Positions  ({source})",
                )

    except ImportError:
        logger.warning("dashboard_renderer not found \u2014 /dashboard unavailable")
        try:
            await status_msg.edit_text(
                "\u274c dashboard_renderer.py not found. "
                "Place it in the project directory to enable /dashboard."
            )
        except Exception:
            pass
    except Exception as e:
...
async def cmd_status_orders(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show order status summary + recent orders with timeline."""
    if not is_authorized(update):
        return
    try:
        with closing(_get_db_connection()) as conn:
            ...
            # Recent orders with timeline (last 10 by most recent status change)
            recent = conn.execute(
            ...
            # Stale detection: STAGED > 1 day
            stale = conn.execute(
                "SELECT id, created_at, payload FROM pending_orders "
        ...
        # Status summary
        _icons = {
            'staged': '\U0001f7e1', 'processing': '\U0001f504',
            'sent': '\u2705', 'acked': '\u2705', 'working': '\u2705',
            'filled': '\u2714\ufe0f', 'partially_filled': '\u2714\ufe0f',
            'failed': '\u274c', 'rejected': '\u26d4',
            'rejected_naked': '\u26d4', 'cancelled': '\u23f9',
            'superseded': '\u23ed\ufe0f', 'duplicate_skipped': '\u23ed\ufe0f',
        }
        for r in count_rows:
            icon = _icons.get(r["status"], '\u2022')
            lines.append(f"{icon} {r['status']}: {r['n']}")

        # Stale alert
        if stale:
            lines.append(f"\n\u26a0\ufe0f {len(stale)} STALE staged order(s) (>1 day):")
            for s in stale[:3]:
                try:
                    p = json.loads(s["payload"])
                    lines.append(f"  #{s['id']} {p.get('ticker','?')} {p.get('action','?')} {s['created_at'][:16]}")
                except Exception:
                    lines.append(f"  #{s['id']} {s['created_at'][:16]}")
        ...
        for r in recent:
            ...
            icon = _icons.get(r["status"], '\u2022')
            ib_id = f" IB#{r['ib_order_id']}" if r["ib_order_id"] else ""
            fill = f" @${r['fill_price']:.2f}x{r['fill_qty']}" if r["fill_price"] else ""

            lines.append(f"{icon} #{r['id']} {ticker} {action} {qty}c{strike_str} → {r['status']}{ib_id}{fill}")

            # Show timeline if status_history exists
            if r["status_history"]:
                try:
                    hist = json.loads(r["status_history"])
                    if len(hist) > 1:
                        timeline = " → ".join(h["status"] for h in hist[-4:])
        ...
        try:
            await update.message.reply_text(
                f"<pre>{html.escape(output)}</pre>", parse_mode="HTML")
        except Exception:
            await update.message.reply_text(output)

    except Exception as exc:
        logger.exception("cmd_status_orders failed")
        try:
            await update.message.reply_text(f"Status check failed: {exc}")
        except Exception:
            pass
...
# ---------------------------------------------------------------------------
# /rollcheck — check roll watchlist positions approaching expiry
# ---------------------------------------------------------------------------

async def cmd_rollcheck(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show active roll watchlist entries with days-to-expiry and spot proximity."""
    if not is_authorized(update):
        return
    try:
        with closing(_get_db_connection()) as conn:
        ...
        if not rows:
            await update.message.reply_text("Roll watchlist is empty.")
            return

        today = _date.today()
        lines = ["\u2501\u2501 Roll Watchlist \u2501\u2501", ""]

        # Batch fetch spot prices (IBKR primary, yfinance fallback)
        ...
        for r in rows:
            ...
            if spot > 0 and strike > 0:
                ...
                proximity = f"spot ${spot:.2f} ({pct_from_strike:+.1f}% to strike)"

            urgency = ""
            if dte <= 3:
                urgency = " \u26a0\ufe0f ROLL NOW"
            elif dte <= 5:
                urgency = " \u23f0 prepare roll"

            mode_tag = r["mode"] or ""
            lines.append(
                f"{ticker} | -{qty}c ${strike:.0f}C {r['expiry']} "
                f"| {dte}d{urgency}"
            )
            if proximity:
                lines.append(f"  {proximity} | {label} | {mode_tag}")
            else:
                lines.append(f"  {label} | {mode_tag}")
            lines.append("")

        lines.append(f"Active positions: {len(rows)}")
        await update.message.reply_text("\n".join(lines))

    except Exception as exc:
        logger.exception("cmd_rollcheck failed")
        try:
            await update.message.reply_text(f"Roll check failed: {exc}")
        except Exception:
            pass
...
async def cmd_cycles(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show wheel cycle history for a ticker."""
    if not is_authorized(update):
        return
    try:
        if not context.args:
            await update.message.reply_text("Usage: /cycles TICKER")
            return

        ticker = context.args[0].upper()
        used_master_log = False

        if READ_FROM_MASTER_LOG:
            try:
                from agt_equities import trade_repo
                trade_repo.DB_PATH = DB_PATH
                # Show for all households
                all_cycles = []
                for hh in HOUSEHOLD_MAP:
                    try:
                        cycles = trade_repo.get_cycles_for_ticker(hh, ticker)
                        all_cycles.extend(cycles)
                    except Exception:
                        pass

                if all_cycles:
                    used_master_log = True
                    lines = [f"\u2501\u2501 Wheel Cycles: {ticker} \u2501\u2501", ""]

                    for c in all_cycles:
                        hh_short = c.household_id.replace("_Household", "")
                        status_icon = "\u2705" if c.status == "ACTIVE" else "\u2b1c"
                        basis_str = f"basis ${c.paper_basis:.2f}" if c.paper_basis else ""
                        adj_str = f"adj ${c.adjusted_basis:.2f}" if c.adjusted_basis else ""
                        prem_str = f"prem ${c.premium_total:.2f}"

                        lines.append(
                            f"{status_icon} Cycle {c.cycle_seq} [{hh_short}] "
                            f"{c.opened_at} \u2192 {c.closed_at or 'ACTIVE'}"
                        )
                        lines.append(
                            f"  {int(c.shares_held)}sh | {basis_str} {adj_str} | {prem_str}"
                        )
                        lines.append(
                            f"  puts={c.open_short_puts} calls={c.open_short_calls} | "
                            f"{len(c.events)} events | rpnl ${c.realized_pnl:.2f}"
                        )
                        lines.append("")

                    lines.append(f"{len(all_cycles)} cycle(s) total")
                    await update.message.reply_text(
                        f"<pre>{html.escape(chr(10).join(lines))}</pre>",
                        parse_mode="HTML",
                    )
            except Exception as exc:
                logger.warning("/cycles master_log fallback: %s", exc)
        ...
        if not used_master_log:
            ...
            with closing(_get_db_connection()) as conn:
                rows = conn.execute(
                    ...
                    SELECT ticker, mode, strike, expiry, bid, annualized,
                           otm_pct, dte, walk_away_pnl, spot, adjusted_basis,
                           created_at, flag
                    FROM cc_cycle_log
                    WHERE ticker = ?
            ...
            if not rows:
                await update.message.reply_text(f"No cycle history for {ticker}.")
                return

            lines = [f"\u2501\u2501 CC Cycles: {ticker} \u2501\u2501", ""]
            consecutive_bad = 0
            for r in rows:
                f = r["flag"] or "NORMAL"
                if f in ("LOW_YIELD", "NO_VIABLE_STRIKE", "SKIPPED"):
                    consecutive_bad += 1
                else:
                    break
            if consecutive_bad >= 3:
                lines.append(
                    f"\U0001f534 {consecutive_bad} consecutive low/skip "
                    f"-- Rule 8 Dynamic Exit evaluation triggered"
                )
                lines.append("")
            for r in rows:
                flag = r["flag"] or "NORMAL"
                mode_short = "DEF" if "DEFENSIVE" in (r["mode"] or "") else "HAR"
                if flag in ("NORMAL", "HARVEST_OK"):
                    icon = "\u2705"
                elif flag == "LOW_YIELD":
                    icon = "\u26a0\ufe0f"
                elif flag in ("NO_VIABLE_STRIKE", "SKIPPED"):
                    icon = "\u274c"
                else:
                    icon = "\u2022"
                if r["strike"]:
                    lines.append(
                        f"{icon} [{mode_short}] ${r['strike']:.0f}C {r['expiry']} "
                        f"@ ${r['bid']:.2f} | {r['annualized']:.0f}% ann | {flag}"
                    )
                else:
                    lines.append(f"{icon} [{mode_short}] {flag}")
                lines.append(f"  {r['created_at'][:16]}")
                lines.append("")
            lines.append(f"Showing last {len(rows)} cycles")
            await update.message.reply_text("\n".join(lines))

    except Exception as exc:
        logger.exception("cmd_cycles failed")
        try:
            await update.message.reply_text(f"Cycles query failed: {exc}")
        except Exception:
            pass
...
async def cmd_fills(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        with closing(_get_db_connection()) as conn:
            rows = conn.execute(
                ...
                SELECT ticker, action, quantity, price,
                       premium_delta, household_id, created_at
                FROM fill_log
                ORDER BY created_at DESC
                LIMIT 20
                """
        ...
        if not rows:
            await update.message.reply_text("No fills recorded yet.")
            return

        lines = ["\u2501\u2501 Recent Fills \u2501\u2501", ""]
        for r in rows:
            delta = r["premium_delta"] or 0
            if delta > 0:
            ...
            hh = (r["household_id"] or "").replace("_Household", "")
            ts = (r["created_at"] or "")[:16]

            lines.append(
                f"{r['ticker']} | {action} {r['quantity']:.0f} @ "
                f"${r['price']:.2f} {delta_str}"
            )
            lines.append(f"  {hh} | {ts}")
            lines.append("")

        lines.append(f"Showing last {len(rows)} fills")
        ...
        logger.exception("cmd_fills failed")
        try:
            await update.message.reply_text(f"Fills query failed: {exc}")
        except Exception:
            pass
...
async def cmd_ledger(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Show position state from Walker cycles (or legacy premium_ledger as fallback)."""
    if not is_authorized(update):
        return
    try:
        rows_data = []
        if READ_FROM_MASTER_LOG:
            try:
                ...
                rows_data.sort(key=lambda r: (r["household_id"], r["ticker"]))
            except Exception as exc:
                logger.warning("/ledger master_log fallback: %s", exc)
                rows_data = []
        ...
        if not rows_data:
            ...
            rows_data = [dict(r) for r in rows] if rows else []

        if not rows_data:
            await update.message.reply_text("No active positions.")
            return

        lines = ["\u2501\u2501 Position Ledger (IRS-adjusted) \u2501\u2501", ""]
        current_hh = None
        for r in rows_data:
            ...
            shares = int(r["shares_owned"] or 0)
            basis = float(r["initial_basis"] or 0)
            prem = float(r["total_premium_collected"] or 0)

            if shares > 0 and basis > 0:
                adj = round(basis - (prem / shares), 2)
                adj_str = f"adj ${adj:.2f}"
            elif shares > 0 and prem > basis * shares:
                adj_str = "FULLY AMORTIZED"
            else:
                adj_str = "no shares"

            lines.append(
                f"{r['ticker']:<6} {shares:>5}sh "
                f"basis ${basis:.2f} "
                f"prem ${prem:.2f} "
                f"| {adj_str}"
            )
        ...
        logger.exception("cmd_ledger failed")
        try:
            await update.message.reply_text(f"Ledger query failed: {exc}")
        except Exception:
            pass
...
async def cmd_dynamic_exit(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Evaluate and STAGE a Dynamic Exit candidate. Usage: /dynamic_exit TICKER [household]"""
    if not is_authorized(update):
        return
    try:
        if not context.args:
            await update.message.reply_text("Usage: /dynamic_exit TICKER [yash|vikram]")
            return
        ...
        if len(context.args) > 1:
            ...
            elif arg in ("vikram", "vik", "v"):
                hh_filter = "Vikram_Household"

        status_msg = await update.message.reply_text(f"Evaluating {ticker}...")

        disco = await _discover_positions(hh_filter)
        if disco.get("error"):
            await status_msg.edit_text(f"Error: {disco['error']}")
            return
        ...
        for hh_name, hh_data in disco["households"].items():
            for p in hh_data["positions"]:
                if p["ticker"] == ticker:
                    result = await _stage_dynamic_exit_candidate(
                        ticker, hh_name, hh_data, p,
                        source="manual_inspection",
                    )
                    results.append(result)

        if not results:
            await status_msg.edit_text(f"{ticker} not found.")
            return

        output = "\n".join(r["summary"] for r in results)
        staged_count = sum(1 for r in results if r["staged"])
        output += f"\n\n{staged_count} candidate(s) STAGED. Review in /cure"
        try:
        ...
        logger.exception("cmd_dynamic_exit failed")
        try:
...
async def cmd_override(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Usage: /override TICKER TIER reason
    Example: /override ADBE HIGH restructuring charge is one-time
    To clear: /override TICKER clear
    """
    if not is_authorized(update):
        return
    try:
        if not context.args or len(context.args) < 2:
            await update.message.reply_text(
                "Usage: /override TICKER TIER reason\n"
                "Example: /override ADBE HIGH one-time restructuring charge\n"
                "To clear: /override TICKER clear"
            )
            return
        ...
        if action == "CLEAR":
            ...
            if result.rowcount > 0:
                ...
                await update.message.reply_text(f"No active override for {ticker}.")
            return

        if action not in ("HIGH", "NEUTRAL", "LOW"):
            await update.message.reply_text("Tier must be HIGH, NEUTRAL, or LOW.")
            return

        if len(context.args) < 3:
            await update.message.reply_text("Justification required. Add reason after tier.")
            return

        justification = " ".join(context.args[2:]).strip()
        ...
        # Get current computed tier for logging
        computed = _compute_conviction_tier(ticker)
        ...
        await update.message.reply_text(
            ...
            f"  {ticker}: {original_tier} \u2192 {action}\n"
            f"  Reason: {justification}\n"
            f"  Expires: {expires_at[:10]}\n"
            f"  Modifier: {CONVICTION_TIERS[original_tier]} \u2192 {CONVICTION_TIERS[action]}"
        )
...
async def cmd_health(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        status_msg = await update.message.reply_text("Loading portfolio health...")
        disco = await _discover_positions(hh_filter, include_staged=False)

        # Fetch per-account PnL from IBKR reqPnL subscription
        # reqPnL provides dailyPnL, unrealizedPnL, and realizedPnL
        account_pnl: dict[str, dict] = {}
        try:
            ib_conn = await ensure_ib_connected()
            pnl_subs: list[tuple[str, object]] = []
            target_accounts: list[str] = []
            for acct_id in ACTIVE_ACCOUNTS:
                if hh_filter and ACCOUNT_TO_HOUSEHOLD.get(acct_id) != hh_filter:
                    continue
                try:
                    pnl_obj = ib_conn.reqPnL(acct_id)
                    pnl_subs.append((acct_id, pnl_obj))
                    target_accounts.append(acct_id)
                except Exception as sub_exc:
                    logger.warning("reqPnL subscribe failed for %s: %s", acct_id, sub_exc)

            # CRITICAL: ib_conn.sleep() processes IB message queue while waiting.
            # asyncio.sleep() does NOT — that's why Vikram was showing $0.
            if pnl_subs:
                for _ in range(6):
                    ib_conn.sleep(0.5)

            for acct_id, pnl_obj in pnl_subs:
                daily = 0.0
                unreal = 0.0
                real = 0.0
                try:
                    if pnl_obj.dailyPnL is not None and not math.isnan(pnl_obj.dailyPnL):
                        daily = float(pnl_obj.dailyPnL)
                    if pnl_obj.unrealizedPnL is not None and not math.isnan(pnl_obj.unrealizedPnL):
                        unreal = float(pnl_obj.unrealizedPnL)
                    if pnl_obj.realizedPnL is not None and not math.isnan(pnl_obj.realizedPnL):
                        real = float(pnl_obj.realizedPnL)
                except Exception as read_exc:
                    logger.warning("reqPnL read failed for %s: %s", acct_id, read_exc)
                account_pnl[acct_id] = {
                    "daily": daily, "unrealized": unreal, "realized": real,
                }
                logger.info(
                    "PnL %s: daily=%.2f unreal=%.2f real=%.2f",
                    acct_id, daily, unreal, real,
                )

            # Cancel all subscriptions
            for acct_id in target_accounts:
                try:
                    ib_conn.cancelPnL(acct_id)
                except Exception:
                    pass
        except Exception as pnl_exc:
            logger.warning("PnL query failed: %s", pnl_exc)

        # Fetch per-ticker P&L from IBKR
        all_active = list(dict.fromkeys(
            acct for hh_name in disco.get("households", {})
            for acct in HOUSEHOLD_MAP.get(hh_name, [])
        ))
        position_pnl = await _fetch_position_pnl(all_active) if all_active else {}

        if disco.get("error"):
            await status_msg.edit_text(f"IB Warning: {disco['error']}")

        all_book_nlv = disco["all_book_nlv"]
        if not disco["households"]:
            await status_msg.edit_text("No positions found.")
            return

        lines: list[str] = []
        for hh_name, hh_data in disco["households"].items():
            nlv = hh_data["household_nlv"]
            margin_nlv = hh_data.get("household_margin_nlv", nlv)
            el = hh_data["household_margin_el"]
            el_pct = hh_data["household_el_pct"]

            # ── Household header ──
            hh_short = hh_name.replace("_Household", "")
            lines.append(f"\u2501\u2501 {hh_short} {'\u2501' * max(1, 22 - len(hh_short))}")
            lines.append(
                f"NLV {_fmt_k(nlv)} (margin {_fmt_k(margin_nlv)}) "
                f"\u00b7 EL {_fmt_k(el)} ({el_pct:.1f}%)"
            )

            # ── Daily P&L line ──
            hh_accounts = HOUSEHOLD_MAP.get(hh_name, [])
            hh_daily = sum(account_pnl.get(a, {}).get("daily", 0.0) for a in hh_accounts)
            hh_unreal = sum(account_pnl.get(a, {}).get("unrealized", 0.0) for a in hh_accounts)
            hh_real = sum(account_pnl.get(a, {}).get("realized", 0.0) for a in hh_accounts)

            day_pct = (hh_daily / nlv * 100) if nlv > 0 else 0.0
            lines.append(
                f"Day {'+' if hh_daily >= 0 else ''}${hh_daily:,.0f} "
                f"({'+' if day_pct >= 0 else ''}{day_pct:.1f}%) "
                f"\u00b7 Unrlzd ${hh_unreal:,.0f} \u00b7 Rlzd ${hh_real:,.0f}"
            )
            lines.append("")
            ...
            for p in hh_data["positions"]:
                weight = round(
                    (p["market_value"] / nlv * 100) if nlv > 0 else 0, 1
                )
                gap_pct = round(
                    ((p["spot_price"] - p["adjusted_basis"]) / p["adjusted_basis"] * 100)
                    if p["adjusted_basis"] > 0 else 0, 1
                )
                gap_arrow = "\u25bc" if gap_pct < 0 else "\u25b2"
                gap_display = f"{gap_arrow}{abs(gap_pct):.1f}%"

                # Line 1: ticker, weight, gap
                lines.append(f"{p['ticker']:<6}{weight:>5.1f}%  {gap_display}")

                # Line 2: basis→spot, shares, available contracts, daily P&L
                basis_short = f"${p['adjusted_basis']:,.0f}"
                spot_short = f"${p['spot_price']:,.0f}"
                avail = p["available_contracts"]
                covered = p["covered_contracts"]
                if covered > 0:
                    cov_str = f"{covered}c/{covered + avail}c"
                else:
                    cov_str = f"{avail}c avail"
                ticker_daily = sum(
                    position_pnl.get(f"{acct}|{p['ticker']}", {}).get("daily", 0.0)
                    for acct in hh_accounts
                )
                mv = p["market_value"]
                day_pct_pos = (ticker_daily / mv * 100) if mv > 0 else 0.0
                day_sign_pos = "+" if ticker_daily >= 0 else ""
                pct_sign_pos = "+" if day_pct_pos >= 0 else ""
                pnl_str = (
                    f" \u00b7 {day_sign_pos}${ticker_daily:,.0f}"
                    f" ({pct_sign_pos}{day_pct_pos:.1f}%)"
                    if ticker_daily != 0.0 else ""
                )
                lines.append(
                    f"  {basis_short}\u2192{spot_short} \u00b7 "
                    f"{p['total_shares']}sh \u00b7 {cov_str}{pnl_str}"
                )

                # Display short calls with unrealized PnL
                for sc in p["existing_short_calls"]:
                    exp_raw = sc["expiry"]
                    try:
                        if "-" in exp_raw:
                            exp_dt = _date.fromisoformat(exp_raw)
                        else:
                            exp_dt = _datetime.strptime(exp_raw, "%Y%m%d").date()
                        exp_short = f"{exp_dt.month}/{exp_dt.day}"
                    except (ValueError, TypeError):
                        exp_short = exp_raw

                    pnl = sc.get("unrealized_pnl")
                    avg_cost = sc.get("avg_cost")
                    if pnl is not None and avg_cost and avg_cost != 0:
                        cost_basis = abs(avg_cost * sc["contracts"])
                        pnl_pct = (pnl / cost_basis * 100) if cost_basis > 0 else 0.0
                        pnl_sign = '+' if pnl >= 0 else ''
                        pnl_str = f" \u00b7 {pnl_sign}${pnl:,.0f} ({pnl_sign}{pnl_pct:.0f}%)"
                    else:
                        pnl_str = ""

                    lines.append(
                        f"  \u270e -{sc['contracts']}c "
                        f"${sc['strike']:.0f}C {exp_short}{pnl_str}"
                    )

                # Display short puts (same format, P instead of C)
                for sp in p.get("existing_short_puts", []):
                    exp_raw = sp["expiry"]
                    try:
                        if "-" in exp_raw:
                            exp_dt = _date.fromisoformat(exp_raw)
                    ...
                    pnl = sp.get("unrealized_pnl")
                    avg_cost = sp.get("avg_cost")
                    if pnl is not None and avg_cost and avg_cost != 0:
                        cost_basis = abs(avg_cost * sp["contracts"])
                        pnl_pct = (pnl / cost_basis * 100) if cost_basis > 0 else 0.0
                        pnl_sign = '+' if pnl >= 0 else ''
                        pnl_str = f" \u00b7 {pnl_sign}${pnl:,.0f} ({pnl_sign}{pnl_pct:.0f}%)"
                    else:
                    ...
                    lines.append(
                        f"  \u270e -{sp['contracts']}p "
                        f"${sp['strike']:.0f}P {exp_short}{pnl_str}"
                    )
            ...
            for p in hh_data["positions"]:
                ...
                if w > 20:
                    breaches_r1.append(f"{p['ticker']} {w:.1f}%")
            if breaches_r1:
                lines.append(f"\u26a0\ufe0f Rule 1: {' \u00b7 '.join(breaches_r1)}")
            else:
                lines.append("\u2705 Rule 1 OK")
            ...
            for p in hh_data["positions"]:
                sec = p["sector"]
                if sec != "Unknown":
                    sector_counts.setdefault(sec, []).append(p["ticker"])
            breaches_r3 = False
            for sec, tickers_list in sector_counts.items():
                if len(tickers_list) > 2:
                    lines.append(
                        f"\u26a0\ufe0f Rule 3: {sec} {len(tickers_list)}x "
                        f"({', '.join(tickers_list)})"
                    )
                    breaches_r3 = True
            if not breaches_r3:
                lines.append("\u2705 Rule 3 OK")

            # ── Residual ──
            residual = [
                p["ticker"] for p in hh_data["positions"]
                if p["sector"] == "Unknown"
            ]
            if residual:
                lines.append(f"\U0001f4ce Residual: {', '.join(residual)}")

            lines.append("")

        # ── All-Book Rule 3 (cross-household) ──
        if len(disco["households"]) > 1:
            lines.append("\u2501\u2501 All-Book Rule 3 \u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501")
            all_sector_counts: dict[str, list[str]] = {}
            for hh_data_ab in disco["households"].values():
                for p in hh_data_ab["positions"]:
                    sec = p["sector"]
                    if sec != "Unknown":
                        entry = f"{p['ticker']}({p['household'][:1]})"
                        all_sector_counts.setdefault(sec, []).append(entry)
            breaches_ab = False
            for sec, entries in all_sector_counts.items():
                if len(entries) > 2:
                    lines.append(
                        f"\u26a0\ufe0f {sec}: {len(entries)}x "
                        f"({', '.join(entries)})"
                    )
                    breaches_ab = True
            if not breaches_ab:
                lines.append("\u2705 All sectors OK cross-household")
            lines.append("")
        ...
        try:
            ...
            # Fallback: split into chunks if too long
            await status_msg.delete()
            for i in range(0, len(output), 4000):
                await update.message.reply_text(output[i:i+4000])
...
async def _walk_mode1_chain(
    ...
    try:
        ...
        # Filter: strikes <= adjusted_basis, sort descending (highest first)
        viable = calls[calls["strike"] <= adjusted_basis].sort_values(
            "strike", ascending=False
        )

        otm_floor = spot * (1 + MODE1_MIN_OTM_PCT / 100)
...
async def _walk_harvest_chain(
    ...
    Strikes walk UP from adjusted_basis. 30-130% annualized band.
    Returns the best strike dict or None.
    """
    try:
        raw_expiries = await _ibkr_get_expirations(ticker)
        today = _date.today()
        ...
        # Filter: strikes >= adjusted_basis, sort ascending (lowest OTM first)
        viable = calls[calls["strike"] >= adjusted_basis].sort_values(
        ...
        for _, row in viable.iterrows():
            ...
            if annualized > HARVEST_MAX_ANNUALIZED_PCT:
                # Too hot — move up to next strike
                continue
...
async def _run_cc_logic(household_filter: str | None = None) -> dict:
    ...
    async def _walk_harvest_target(p):
        ...
        result = await _walk_chain_limited(
            _walk_harvest_chain, ticker, spot, adj_basis, HARVEST_TARGET_DTE
        )
        if result is None:
...
async def cmd_cc(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        status_msg = await update.message.reply_text(
            "Walking CC chains (defensive + harvest) and staging..."
        )

        result = await _run_cc_logic(hh_filter)
        ...
        try:
            await update.message.reply_text(f"CC staging failed: {exc}")
        except Exception:
            pass


# Backward compat — /mode1 still works
cmd_mode1 = cmd_cc
...
async def cmd_reconcile(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        from agt_equities.walker import walk_cycles as _wc, UnknownEventError as _UE, get_walker_warnings as _gww
        ...
        lines = [
            ...
            f"Last sync: #{last_sync['sync_id']} {last_sync['finished_at']} ({last_sync['status']}, {last_sync['rows_inserted']} rows)",
            f"Parity: {eae_total - len(violations)}/{eae_total}",
            f"Cycles: {len(all_cycles)} ({active} active, {wheel} wheel, {satellite} satellite)",
            f"Frozen: {frozen_count}",
            "",
            f"A (realized P&L): {a_ok}/{a_ok + a_div}",
        ]
        for d in a_divergent:
            lines.append(f"  {d}")
        lines.extend([
            f"B (cost basis): {b_ok}/{b_ok + b_div}",
            f"C (NAV recon): {c_ok}/{c_ok + c_div}",
        ])
        for d in c_details:
            lines.append(f"  {d}")
        ...
        try:
            ...
            if t2_orphans or stale:
                lines.append("")
                lines.append("ORDER HEALTH:")
            if t2_orphans:
                lines.append(f"  T+2 orphans: {len(t2_orphans)} (filled but no Flex match)")
                for o in t2_orphans[:3]:
                    lines.append(f"    #{o['id']} IB#{o['ib_order_id']} {o['status']} {o['created_at'][:16]}")
            if stale:
                lines.append(f"  Stale staged: {len(stale)} (>1 day old)")
            if not t2_orphans and not stale:
                lines.append("")
                lines.append("ORDER HEALTH: clean")
        except Exception:
            pass

        # W3.6: Walker warnings section (live from accumulated warnings)
        try:
            if not all_walker_warnings:
                lines.append("")
                lines.append("WALKER WARNINGS: 0 \u2705")
            else:
                _sev_emoji = {"ERROR": "\U0001F534", "WARN": "\U0001F7E1", "INFO": "\u2139\uFE0F"}
                lines.append("")
                lines.append(f"WALKER WARNINGS: {len(all_walker_warnings)}")
                for _w in all_walker_warnings[:20]:
                    _e = _sev_emoji.get(_w.severity, "\u2753")
                    _loc = "/".join(filter(None, [_w.household, _w.ticker]))
                    lines.append(f"  {_e} [{_w.code}] {_loc}: {_w.message}")
                if len(all_walker_warnings) > 20:
                    lines.append(f"  ... +{len(all_walker_warnings) - 20} more")
        except Exception:
            lines.append("")
            lines.append("WALKER WARNINGS: \u26A0\uFE0F walker unavailable")
...
async def cmd_clear_quarantine(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/clear_quarantine TICKER — remove corp action quarantine for a ticker."""
    if not is_authorized(update):
        return
    if not context.args:
        await update.message.reply_text("Usage: /clear_quarantine TICKER")
        return
    ticker = context.args[0].upper()
    try:
        with closing(_get_db_connection()) as conn:
