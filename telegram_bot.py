...
# Dynamic Exit payloads are now generated by _generate_dynamic_exit_payload()
...
async def _generate_dynamic_exit_payload(
    ticker: str,
    hh_name: str,
    hh_data: dict,
    position: dict,
    ...
    """
    Generate a complete V7 Dynamic Exit CIO payload.
    Returns formatted string ready for Telegram + CIO paste.
    """
    try:
        ...
        lines = []
        lines.append(f"\u2501\u2501 Dynamic Exit: {ticker} ({hh_short}) \u2501\u2501")
        lines.append("")

        # Position
        lines.append(f"Position: {shares}sh @ ${spot:.2f} = ${position_value:,.0f}")
        lines.append(f"Concentration: {position_pct:.1f}% of ${hh_nlv:,.0f}")
        lines.append(f"Basis: ${adj_basis:.2f} | Gap: {((spot - adj_basis) / adj_basis * 100):+.1f}%" if adj_basis > 0 else f"Basis: ${adj_basis:.2f}")
        lines.append(f"Escalation: {escalation['tier']} ({escalation['frequency']})")
        lines.append("")

        # Conviction
        lines.append(f"Conviction: {conviction['tier']} (x{modifier})")
        if conviction.get("source") == "CIO_OVERRIDE":
            lines.append(f"  SOURCE: CIO Override (expires {conviction.get('expires', '?')})")
            lines.append(f"  Reason: {conviction.get('justification', '')}")
        else:
            inputs = conviction.get("inputs", {})
            lines.append(f"  EPS: {inputs.get('eps_revision_trend', '?')}")
            lines.append(f"  Revenue: {inputs.get('revenue_growth_vs_sector', '?')}")
            lines.append(f"  Analyst: {inputs.get('analyst_consensus_shift', '?')}")
            lines.append(f"  Margins: {inputs.get('margin_trend', '?')}")
        lines.append("")

        # Scope
        if scope["scope"] == "DRAWDOWN_EXCEPTION":
            lines.append(f"Exit Scope: DRAWDOWN EXCEPTION")
            lines.append(f"  {scope['reason']}")
            lines.append("")
            lines.append("No Dynamic Exit recommended. Position recovers via Mode 1 CCs.")
            return "\n".join(lines)

        if scope["scope"] == "OVERWEIGHT_ENCUMBERED":
            lines.append("Exit Scope: OVERWEIGHT_ENCUMBERED")
            lines.append(
                f"  {scope['excess_shares']}sh excess but 0c available "
                f"-- existing short calls must expire or roll first"
            )
            covered = position.get("covered_contracts", 0)
            lines.append(
                f"  Existing CCs: {covered}c covering {covered * 100}sh"
            )
            for sc in position.get("existing_short_calls", []):
                exp_raw = sc.get("expiry", "")
                lines.append(
                    f"  \u270e -{sc['contracts']}c ${sc['strike']:.0f}C {exp_raw}"
                )
            lines.append("")
            lines.append("ACTION: Let existing CCs expire, then re-evaluate.")
            return "\n".join(lines)

        lines.append(f"Exit Scope: {scope['scope']}")
        lines.append(f"  Target: {scope['target_pct']:.0f}% = {scope['target_shares']}sh")
        lines.append(f"  Excess: {scope['excess_shares']}sh = {excess_contracts}c")

        # Show existing encumbrance for operator transparency
        existing_ccs = position.get("existing_short_calls", [])
        covered = position.get("covered_contracts", 0)
        avail = position.get("available_contracts", 0)
        total_contracts = position.get("total_shares", 0) // 100
        if covered > 0:
            lines.append(f"  Contracts: {total_contracts}c total = {covered}c encumbered + {avail}c available")
            for sc in existing_ccs:
                exp_str = sc.get("expiry", "?")
                lines.append(
                    f"    \u270e -{sc['contracts']}c ${sc['strike']:.0f}C {exp_str}"
                )
        else:
            lines.append(f"  Contracts: {total_contracts}c total (all available)")

        lines.append(f"  After: {scope['remaining_shares']}sh ({scope['remaining_pct']:.1f}%)")
        lines.append("")

        # Gate 1 at each viable strike
        if excess_contracts > 0:
            lines.append(f"Gate 1 (x{modifier}):")
            try:
                expiries = await _ibkr_get_expirations(ticker)
                if not expiries:
                    lines.append("  No expirations available. Try /dynamic_exit manually.")
                    return "\n".join(lines)
                today_d = _date.today()
                ...
                for exp_str in expiries:
                    try:
                        ...
                        if 14 <= dte <= 30 and abs(dte - 21) < abs(best_dte - 21):
                            best_exp = exp_str
                            best_dte = dte
                    except ValueError:
                        continue
                ...
                if best_exp:
                    chain = await _with_timeout_async(yf_tkr.option_chain, best_exp, timeout=15)
                    if chain is None:
                        lines.append("  Option chain timed out. Try /dynamic_exit manually.")
                        return "\n".join(lines)
                    ...
                    for _, row in viable.head(5).iterrows():
                        ...
                        if wa_per_share >= 0:
                            # Profitable exit — Gate 1 auto-passes
                            gate1_pass = True
                            walk_away = 0
                            ratio = 999
                        else:
                            ...
                            gate1_pass = velocity > walk_away
                            ratio = (velocity / walk_away) if walk_away > 0 else 999

                        if wa_per_share >= 0:
                            icon = "\u2705"
                            lines.append(
                                f"  {icon} ${strike:.0f}C @ ${bid:.2f} | "
                                f"PROFITABLE (+${wa_per_share * 100 * excess_contracts:,.0f}) "
                                f"Gate 1 AUTO-PASS"
                            )
                        else:
                            icon = "\u2705" if gate1_pass else "\u274c"
                            lines.append(
                                f"  {icon} ${strike:.0f}C @ ${bid:.2f} | "
                                f"${velocity:,.0f} vs ${walk_away:,.0f} ({ratio:.1f}x)"
                            )

                        if gate1_pass and ratio > best_ratio:
                            best_ratio = ratio
                            best_strike = strike
                            best_bid = bid

                    lines.append(f"  Expiry: {best_exp} ({best_dte}d)")
                    lines.append("")

                    if best_strike:
                        lines.append("CIO: If approved, respond with:")
                        lines.append(
                            f"  /exit {ticker} {best_strike:.0f} {best_exp} "
                            f"{excess_contracts} {hh_short.lower()} {best_bid:.2f}"
                        )
                    else:
                        lines.append("All strikes FAIL Gate 1. EXIT REJECTED.")
                else:
                    lines.append("  No viable expiry in 14-30 DTE window")
            except Exception as exc:
                lines.append(f"  Chain error: {exc}")
        elif scope["scope"] == "OVERWEIGHT_SUB_LOT":
            lines.append("Gate 1: N/A -- overweight < 100sh (sub-lot)")
            lines.append("Must recover naturally or via Drawdown Exception.")
        else:
            lines.append("Gate 1: N/A -- no excess contracts")

        return "\n".join(lines)
    except Exception as exc:
        logger.exception("_generate_dynamic_exit_payload failed for %s", ticker)
        return f"Dynamic Exit payload failed for {ticker}: {exc}"


async def cmd_dynamic_exit(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Generate V7 Dynamic Exit CIO payload. Usage: /dynamic_exit TICKER [household]"""
    if not is_authorized(update):
        return
    try:
        if not context.args:
        ...
        for hh_name, hh_data in disco["households"].items():
            for p in hh_data["positions"]:
                if p["ticker"] == ticker:
                    payload = await _generate_dynamic_exit_payload(
...
async def _run_cc_logic(household_filter: str | None = None) -> dict:
    ...
    dynamic_exit_payloads: list[str] = []
    ...
    for hh_name, hh_data in disco["households"].items():
        ...
        for p in hh_data.get("positions", []):
            ...
            if scope["excess_contracts"] > 0:
                ...
                # Generate CIO payload
                try:
                    payload = await _generate_dynamic_exit_payload(
                        p["ticker"], hh_name, hh_data, p
                    )
                    dynamic_exit_payloads.append(payload)
                except Exception as de_exc:
                    logger.warning("Dynamic exit payload failed for %s: %s",
                                   p["ticker"], de_exc)
    ...
    if dynamic_exit_payloads:
        lines.append(f"\u2501\u2501 Dynamic Exits: {len(dynamic_exit_payloads)} payloads (sent separately) \u2501\u2501")
        lines.append("")
                ...
                for hh_name, hh_data in disco.get("households", {}).items():
                    ...
                    for p in hh_data.get("positions", []):
                        ...
                        if days_overweight >= required_days:
                            ...
                            hh_short = hh_name.replace("_Household", "")
                            alerts.append(
                                f"\U0001f6a8 DYNAMIC EXIT \u2014 {p['ticker']} "
                                f"({hh_short}) at {position_pct:.1f}% "
